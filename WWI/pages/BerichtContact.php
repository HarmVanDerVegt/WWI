...
if (isset(filter_input(INPUT_POST, voornaam, FILTER_SANITIZE_STRING))) {
    if (isset(filter_input(INPUT_POST, achternaam, FILTER_SANITIZE_STRING))) {
        if (isset(filter_input(INPUT_POST, email, FILTER_SANITIZE_STRING))) {
            if (isset(filter_input(INPUT_POST, bericht, FILTER_SANITIZE_STRING))) {
                if (isset(filter_input(INPUT_POST, onderwerp, FILTER_SANITIZE_STRING))) {
                    ?>
                    <p><h3>Hartelijk dank voor uw bericht, kunnen we u verder helpen?</h3></p>
                    <a class="btn btn-sample" href="\WWI\WWI\pages\index.php">Terug naar startpagina...</a>
