<!--<br>
<!-- <p class="text-center">Algemene voorwaarden Privacy Cookies © 2018 Wide World Imports</p> -->
<head>
    <meta charset="UTF-8">
    <link href="\WWI\WWI\css\footer.css" rel="stylesheet" type="text/css"/>
    <title></title>
</head>
<!-- Footer -->
<footer class="page-footer font-small bg-light">

    <div class="footer-custom">
        <div class="container" class="footer-custom">

            <div class="text-center py-3 text-white"> <a href="/WWI/WWI/pages/Register.php">Registreer</a>      <a href="/WWI/WWI/pages/Voorwaarden.php">Voorwaarden</a>          <a href="/WWI/WWI/pages/Contact.phh">Contact</a></div>
        </div>
    </div>

    <!-- Copyright -->
    <div class="footer-copyright text-center py-3">© 2018 Copyright: Wide World Importers
    </div>
    <!-- Copyright -->

</footer>
<!-- Footer -->